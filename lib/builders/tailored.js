...
// `pageBreakBefore` forces a hard page break so the entry starts on a new
// page (used for Bank of America, which the candidate wants on page 2).
const jobHeader = (company, location, dates, { pageBreakBefore = false } = {}) => new Paragraph({
  spacing: { before: pageBreakBefore ? 0 : 140, after: 30 },
...
// Companies whose section should always start on a new page. Today this is
// only Bank of America (per the candidate's preference — keeps Discover /
// CapitalOne and the page-1 sections together, then BoA opens page 2).
// Matching is case-insensitive and forgiving (substring match).
const PAGE_BREAK_COMPANIES = [/bank of america/i];

function shouldPageBreakBefore(company) {
  if (typeof company !== 'string') return false;
  return PAGE_BREAK_COMPANIES.some((re) => re.test(company));
}
...
export function buildDocument(tailoredResume) {
  ...
  if (experience.length > 0) {
    ...
    for (const job of experience) {
      const pageBreakBefore = shouldPageBreakBefore(job.company);
      children.push(jobHeader(job.company, job.location ?? null, job.dates, { pageBreakBefore }));
